...
//     // Borç hesaplaması (Sadece bakiyesi 0'dan büyük olanlar)
...
//             // 2. ÖDEME DAĞILIMI (GÖRÜNTÜDEKİ HATALI KISIM BURASIYDI)
//             salesHistory.when(
//               data: (sales) => _buildPaymentDistribution(sales),
...
//               crossAxisAlignment: CrossAxisAlignment.start,
//               children: [
//                 // SOL TARAF: AYARLAR VE KRİTİK STOK
//                 Expanded(
//                   flex: 1,
//                   child: Column(
//                     children: [
...
//                 // SAĞ TARAF: DETAYLI BORÇLU LİSTESİ
//                 Expanded(
//                   flex: 1,
...
//   // --- ÖDEME DAĞILIMI HESAPLAMA VE GÖSTERİM ---
//  Widget _buildPaymentDistribution(List<Map<String, dynamic>> sales) {
//   double n = 0, k = 0, v = 0;

//   for (var s in sales) {
//     String m = (s['paymentMethod'] ?? "").toString().toUpperCase();
//     double a = (s['totalAmount'] as num).toDouble();
//     if (m.contains("KART")) k += a;
//     else if (m.contains("VERESİYE")) v += a;
//     else n += a;
//   }

//   return Column(
//     children: [
//       _paymentRow("Nakit Tahsilat", n, Colors.greenAccent, Icons.money),
//       const SizedBox(height: 15),
//       _paymentRow("Kredi Kartı", k, Colors.blueAccent, Icons.credit_card),
//       const SizedBox(height: 15),
//       _paymentRow("Veresiye / Borç", v, Colors.orangeAccent, Icons.handshake),
//       // BURADAN SONRA ASLA BAŞKA BİR _paymentRow VEYA DÖNGÜ OLMAMALI!
//     ],
//   );
// }

//   Widget _paymentRow(String title, double amount, Color color, IconData icon) {
//     return Row(
//       children: [
...
//         const Spacer(),
//         Text("₺${amount.toStringAsFixed(2)}", style: GoogleFonts.ubuntu(color: Colors.white, fontWeight: FontWeight.bold)),
//       ],
//     );
//   }
...
//                     child: Text(c.name.isNotEmpty ? c.name[0] : "?", style: const TextStyle(color: Colors.orange, fontSize: 14))
//                   ),
//                   const SizedBox(width: 15),
//                   Expanded(child: Text(c.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w500))),
...
//   // --- DİALOGLAR VE YARDIMCILAR (Aynı Kaldı) ---
...
class SummaryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFF020617),
      appBar: AppBar(
        title: const Text("Günlük Özet", style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.transparent,
        elevation: 0,
      ),
      body: salesAsync.when(
        data: (sales) => SingleChildScrollView(
          padding: const EdgeInsets.all(16),
          child: Column(
            children: [
              // ÖDEME DAĞILIMI BURADA ÇAĞRILIYOR
              _buildPaymentDistribution(sales),
            ],
          ),
        ),
  ...
  Widget _buildPaymentDistribution(List<Map<String, dynamic>> sales) {
    // 1. TOPLAMLARI HESAPLA (Döngü sadece burada olur)
    double nakit = 0.0;
    double kart = 0.0;
    double veresiye = 0.0;

    for (var sale in sales) {
      String method = (sale['paymentMethod'] ?? "NAKİT").toString().trim().toUpperCase();
      double amount = (sale['totalAmount'] as num).toDouble();

      if (method == "NAKİT") {
        nakit += amount;
      } else if (method.contains("KART") || method == "K.KARTI") {
        kart += amount;
      } else if (method == "VERESİYE") {
        veresiye += amount;
      }
    }

    // 2. SADECE 3 TANE STATİK SATIR DÖNDÜR
    return Container(
      padding: const EdgeInsets.all(25),
      decoration: BoxDecoration(
        color: const Color(0xFF0F172A),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 25),
          // BU 3 SATIR ASLA ÇOĞALMAZ
          _paymentRow("Nakit Tahsilat", nakit, Colors.greenAccent, Icons.money),
          const SizedBox(height: 20),
          _paymentRow("Kredi Kartı", kart, Colors.blueAccent, Icons.credit_card),
          const SizedBox(height: 20),
          _paymentRow("Veresiye / Borç", veresiye, Colors.orangeAccent, Icons.handshake),
        ],
      ),
    );
  }
    ...
    return Column(
      children: [
        ...
        LinearProgressIndicator(
          value: amount > 0 ? 1.0 : 0.0, // Basit doluluk oranı
